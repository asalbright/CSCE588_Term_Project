...
At 1000 episodes the mean rewards of both algorithms are similar, both within one standard deviation of either algorithm's performance. However, PPO shows to be the preferable algorithm as it has a significantly smaller standard deviation.
This means that fewer agents would need to be trained using PPO, as training an agent using the PPO algorithm will more consistently lead to the algorithms optimal result.  \todo[inline]{what are the differences though?}

\todo[inline]{what are the differences though?}
%
\begin{figure}[t]
        \begin{center}
...
These figures show that the PPO and TD3 Algorithm on average converge to a similar result. However, the variation in the standard deviation of the parameter choices between the PPO agents, show that each agent's parameter choice varied greatly between episodes. This implies a higher preference for exploration in the PPO agents than the TD3. The larger standard deviation of the TD3 algorithms does not demonstrate the algorithm's preference for exploration, instead it shows that agents trained with TD3 algorithm converged on solutions with more significant differences.
...
\todo[inline]{This needs some rewriting.}
...
\begin{minipage}[b]{.45\textwidth}
        ...
        \caption{TD3 Reward}\label{label-b}
\end{minipage}
\end{figure*}
        
In the process of this project the effects of learning rate on the PPO and TD3 Algorithms were explored.
Figures~\ref{label-a} and \ref{label-b} show that the Time Steps needed for the PPO algorithm to converge is very sensitive to the choice in learning rate, it also shows
that the overall mean and standard deviation of the rewards is sensitive. On the other hand, Td3 does not appear to be at all sensitive to changes in the learning rate, at least not in the range
of learning rates explored in this study.

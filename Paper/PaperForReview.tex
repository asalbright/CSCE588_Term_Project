...
Figures~\ref{fig:spring_zeta_height} represents the height the monopode could reach for the design/action space. The design space provided represents a space where a wider range of damping ratios are allowed. This wider range of possible values makes it more likely that the agent will settle to a local maxima. \todo[inline]{something about how this can be used to visually instect the learned parameters.}

\subsection{Design Learned}

Figure~\ref{fig:rew_vs_step} shows the rewards the agents received during training and support that both algorithm types learned designs which converged. \todo[inline]{what are the differences though?}
%
\begin{figure}[t]
        \begin{center}
...
The average and standard deviation of the spring constant and damping ratio design parameters the agents selected during training are shown in Figures~\ref{fig:spring_vs_step} and~\ref{fig:zeta_vs_step}. \todo[inline]{what are the differences though?}
